$(document).ready(function () {
  ...
  $(window).scroll(function () {
    ...
    $("section article").each(function (i) {
      //each :각각의 기능을 넣는다.
      var top = $(this).offset().top - 600; //내가 선택한 article의 위치를 각각 변수에 대입
  ...
  gnb.click(function () {
    ...
    $("html, body").animate(
      {
        scrollTop: $("section article").eq(n).offset().top,
      },
      500,
      "easeInOutExpo"
    );
  ...
  Object.keys(skillLevel).forEach(function (skill) {
    var skillElement =
      '<li id="' +
      skill +
      '">' +
  ...
  $(window).scroll(
    _.debounce(function () {
      ...
      if (sPos >= minRange && sPos <= maxRange) {
        Object.keys(skillLevel).forEach(function (key) {
  ...
  $(".project-box").hover(
    function () {
      $(this).find("img").stop().animate(
        { top: "-1000px" }, // 긴 이미지라면 원하는 만큼 위로
        5000
      );
    },
    function () {
      $(this).find("img").stop().animate(
        { top: "30px" }, // 다시 윈도우바 밑으로
        300
      );
    }
  );
  ...
  $(".g_list li").click(function () {
    ...
    const modal =
      ...
      src +
      '" alt=""><span class="x_btn">X</span></div>';
